...
interface BgBlurProps {
  ...
  opacity?: number; // Opacity of the blur effect
  imgUrl?: string; // URL of the background image (if any)
}
...
// Extract props values
  ...
  // Handle case without background image
  return {
    backdropFilter: `blur(${blur}px)`,
    WebkitBackdropFilter: `blur(${blur}px)`,
    backgroundColor: alpha(color, opacity),
...
// Assume that `alpha` is imported correctly
// const alpha = (color: string, value: number) => color; // Replace this with the actual implementation
